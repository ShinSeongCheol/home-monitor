...
import { EditCommentModal, InsertCommentModal } from '../../../components/BackOfficeModal.tsx';
        ...
        <section className={`${styles.section}`}>
            <div className={styles.container}>
                ...
                <div className={`${styles.buttonGroup}`}>
                    ...
                    <CsvButton svg={<Download color='white' size={16} strokeWidth={2}/>} value='CSV' type='button' onClick={() => agGridComponentRef.current?.api.exportDataAsCsv({fileName: `댓글 목록 ${formattedDate}.csv`})}/>
                </div>
                <AgGridReactComponent ref={agGridComponentRef} colDefs={colDefs} rowData={rowData}></AgGridReactComponent>
                <InsertCommentModal isOpen={isInsertModalOpen} setIsOpen={setIsInsertModalOpenOpen} fetchData={fetchData}/>
                <EditCommentModal isOpen={isEditModalOpen} setIsOpen={setIsEditModalOpenOpen} fetchData={fetchData} data={editModalData}/>
            </div>
        </section>
    )
}